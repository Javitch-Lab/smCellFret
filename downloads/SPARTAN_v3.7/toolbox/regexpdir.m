...
for i = find(~[dirtree.isdir]);
    if regexpi(strrep(fullfile(rootdir, dirtree(i).name), basedir, ''), expstr);
        % Add the full path to the file name and add it to the list.
        dirtree(i).name = fullfile(rootdir, dirtree(i).name);
        dirlist = [dirlist; dirtree(i)];
    end
end
...
% 12-11-2014 v1.02 (DT)   Return dir struct instead of path strings.
%==========================================================================
